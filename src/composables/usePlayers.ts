...
export const usePlayers: Function = (): UsePlayers => {
  const addPlayerPoints = (id: number, points: number): void => {
    const playerIndex: number = players.value.findIndex(
      (player) => player.id === id
    )
  ...
  const subtractPlayerPoints = (id: number, points: number): void => {
    const playerIndex: number = players.value.findIndex(
      (player) => player.id === id
    )
    if (players.value[playerIndex].score + points > 501) return
    players.value[playerIndex].score += points
